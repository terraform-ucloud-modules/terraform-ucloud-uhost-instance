...
resource "ucloud_eip" "eips" {
  ...
  duration      = "${var.duration}"
  name          = "${var.eip_count < 2 ? var.eip_name : format("%s-%s", var.eip_name, format(var.count_format, count.index+1))}"
  remark        = "${var.eip_remark}"
  tag           = "${var.eip_tag}"

  count = "${var.eip_count * var.instance_count}"
}
...
resource "ucloud_eip_association" "default" {
  ...
  count       = "${(var.instance_count > 0 && var.eip_count > 0) ? var.eip_count * var.instance_count : 0}"
}

resource "ucloud_disk" "disks" {
  availability_zone = "${var.availability_zone == "" ? data.ucloud_zones.default.zones.0.id : var.availability_zone}"
  name              = "${var.disk_count < 2 ? var.disk_name : format("%s-%s", var.disk_name, format(var.count_format, count.index+1))}"
  disk_size         = "${var.disk_size}"
  disk_type         = "${var.disk_type}"
  charge_type       = "${var.charge_type}"
  duration          = "${var.duration}"
  tag               = "${var.disk_tag}"
  count             = "${var.disk_count * var.instance_count}"
}
...
resource "ucloud_disk_attachment" "default" {
  ...
  count             = "${(var.disk_count > 0 && var.instance_count > 0) ? var.disk_count * var.instance_count: 0}"
}
